# Version 10/28/2024
# Use Descartes' for find_d_center instead of my original Cramer's thing
# Test #4 works, but Test #1 doesn't and Test #5's center seems wrong
# Changed tolerance to 1e-7 because of increased error margins with Descartes evaluations
# Modified selecting of find_d_center() return value in find_circD()
# Test #1 works
# Tests #2-4 work
# I forgot to change the cd in Descartes to -cd; test #5's center now works :)
# Tests #1-4 work
# Tests #5 and #6 work - change tolerance to 1 for those two
...
def rd_works(circA, circB, circC, rd, center):
    ...
    (hd,kd) = center
    print("Running rd_works for rd = " + str(rd) + ", center " + str((hd,kd)))
...
def find_circD(circA, circB, circC):
    ...
    if len(rds)!= 1:     # if there isn't only one possibility for rd left
        ...
        rd = rds[0]
        (hd,kd) = find_d_center(circA, circB, circC, rd)[0]   # added [0] bc now that error tolerance is larger find_d_center could return two identical solutions with very minimal differences so just choose 1 of the 2
...
def graph_first_four(circA, circB, circC, circO):
    ...
    if centD == None:   # since it's both or neither I really only have to check one of them
        ...
        print("Plotting circle D: center " + str(centD) + " radius " + str(rd))   # for testing
...
### Test Case #3
##r = 1
##circA = myCircle((1,2),r,gasketInv)
##circB = myCircle((1,0),r,gasketInv)
##circC = myCircle((1,-2),r,gasketInv)
##
####circA.plot()
####circB.plot_all()
####circC.plot_all()
...
### Test Case #4
##r = 1
##circA = myCircle((1+0.707106781186548,0-0.707106781186548),r,gasketInv)
##circB = myCircle((1+math.sqrt(2)+0.707106781186548,math.sqrt(2)-0.707106781186548),r,gasketInv)
##circC = myCircle((1-math.sqrt(2)+0.707106781186548,-1*math.sqrt(2)-0.707106781186548),r,gasketInv)
##
####circA.plot()
####circB.plot_all()
####circC.plot_all()
