# Version 10/29/2024
# Deleted the original commented out code for Cramer's Rule
# Added find_unscaled_ABC
# Modified initial circ functions to adjust to user input of radii instead of circles
# Test #1 works
# Added parameter to take in "straight" case: e.g. rd = inf
# Test #2 and #1 work
# Deleted test #3 because without orientation it's the same as test #2
# Tests #4, 5, and 6 work
# Note: actually, test #6 doesn't work
...
def find_unscaled_ABC(ra,rb,rc):
    ...
    (hc,kc) = (solutions[0][x],solutions[0][y])   # there will be 2 solutions but doesn't matter which because gasket is same
