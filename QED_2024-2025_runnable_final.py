# Version 11/27/2024
# Removed all commented print statements as well as other obsolete commented code
# Added user input
# Deleted check for whether it is the initial three circles
...
# Get input from user
amnt = int(input("Enter the amount of points to take on each figure. The more points, the more precision; recommended = 2000. "))
r1 = input("Enter the first radius. ")
r2 = input("Enter the second radius. ")
r3 = input("Enter the third radius. ")
config = input("Straight or circular configuration? (s/c) ")
n = int(input("Number of layers? Five recommended for optimal runtime. "))
graph = input("Graph the Ford circles? (y/n) ")

# To take care of entering radii as 1/k
def to_float(r):
    if "/" in r:
...
tDist = 15
tValues = np.linspace(0,tDist,amnt)  # for getting tons of close points to graph parametrics
...
def rd_within_ABC_cents(centA,centB,centC,centD):
    ...
    yesAB = kd - ka <= (ka-kb)/(ha-hb) * (hd - ha) + tolerance   # if centD is within bounds of line from centA to centB
    yesBC = kd - kb >= (kc-kb)/(hc-hb) * (hd - hb) - tolerance   # if centD is within bounds of line from centC to centB
    yesAC = kd - ka >= (ka-kc)/(ha-hc) * (hd - ha) - tolerance   # if centD is within bounds of line from centA to centB
...
if graph == "y":
    ...
    raise ValueError("Only enter y/n for graphing condition.")

print("Running...")
generate_gasket(to_float(r1),to_float(r2),to_float(r3),gasketInv,layers=n,config=configur,graphFord=shouldGraph)
...
##generate_gasket(r,r,r,gasketInv,config="straight",graphFord=False)
...
##generate_gasket(1/10,1/15,1/19,gasketInv,layers=5,graphFord=False)
