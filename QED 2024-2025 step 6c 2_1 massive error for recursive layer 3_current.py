# Version 10/31/2024
# Deleted unnecessary plot_all()s and circ definitions still in test case code
# Moved Ford Circles section location more to bottom
# Changed find_third_radius, find_third_center, and find_third_circ variable namings to P,Q,R so as to fit new naming conventions
# Added create_Ford_Circles
# Added loc parameters to find_third_center and find_third_circ
# Changed loc to vloc and added hloc parameters to find_third_center and find_third_circ
# Added find_Dprime()
# Added find_rdp()
# Added find_Dp_center()
# Added Dp_cent_works()
# Added find_three_circs() (originally called find_three in rough draft of code on pg. 40 of research records)
# Added next_three()
...
def find_rdp(circ1,circ2,circ3):
    '''Returns the radius of the circle externally tangent to 3 other surrounding circles;
    ...
    if len(solutions) != 1:
        raise ValueError("There isn't only one possible solution for rdp. The solutions are: " + str(solutions))
    return solutions[0]

def Dp_cent_works(circ1, circ2, circ3, rdp, center):
    '''Returns a boolean for if a certain Dp center works; run in find_Dp_center()'''

    (hdp,kdp) = center
    print("Running Dp_cent_works for rdp = " + str(rdp) + ", center " + str((hdp,kdp)))   # for testing
...
def find_Dp_center(lim1,lim2,lim3,rdp):
    '''Returns the a tuple of center coordinates of the circle externally tangent to the three limits;
    ...
    print("\nDp center found: " + str(solutions[0]) + "\n")
    return solutions[0]
...
def find_Dprime(lim1,lim2,lim3,lim3Type="circle"):
    ...
    if lim3Type == "line":   # if lim3 is the top or bottom line
        ...
        r = find_rdp(lim1,lim2,lim3)
        (h,k) = find_Dp_center(lim1,lim2,lim3,r)
...
def find_three_circs(currCirc,limsList,lim3Type,circO):
    ...
    for limpair in [(1,3),(1,2),(2,3)]:   # numbers represent lim#; e.g. 1 = lim1, 2 = lim2 etc.; in order that would yield circs T,U,V
        ...
        print("limOrders in find_three_circles: " + str(limOrders))  # testing
...
# Ford Circle Set
create_Ford_Circles(gasketInv,1)
